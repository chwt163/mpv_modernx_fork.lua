...
local osc_styles = {
    ...
    rjno160Buttons = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs56\\fnmpv-osd-symbols}",
    rjno150Buttons = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs46\\fnmpv-osd-symbols}",
    rjno140Buttons = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs36\\fnmpv-osd-symbols}",
    rjno135Buttons = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs30\\fnmpv-osd-symbols}",
    ...
    timecodes = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs25}",
...
layouts["box"] = function ()
    ...
    lo.geometry = {x = posX, y = posY + 13, an = 5, w = osc_w, h = osc_h + 13}
    lo.layer = 10
...
--box左上角y坐标 posY - pos_offsetY 但an=5 元素中心点坐标和这个坐标一致，所以还要根据an=？加减元素的高度
    ...
    -- Seekbar
    --
	geo ={x = rjno1X + 20, y = rjno1Y + 20 , an = 1, w = osc_geo.w - 40, h = 13}
...
--    lo.style = osc_styles.rjno130Buttons	
   ...
   lo.geometry = {x = posX -200, y = rjno1Y + 10 * 1.6 + 80 / 2 , an = 5, w = 30, h = 50}
   lo.style = osc_styles.rjno135Buttons	
	
    lo = add_layout("skipback")
    lo.geometry = {x = posX -100, y = rjno1Y + 10 * 1.6 + 80 / 2 , an = 5, w = 30, h = 50}
    lo.style = osc_styles.rjno140Buttons	

	
    lo = add_layout("playpause")
    lo.geometry = {x = posX, y = rjno1Y + 10 * 1.6 + 80 / 2 , an = 5, w = 30, h = 50}
    lo.style = osc_styles.rjno160Buttons		
...
--an = 5 时文字居中显示 往2边增加，所以一定要用1，这样多出的文字只会往右侧增长
	lo = add_layout("title")
    lo.geometry = {x = rjno1X +40, y = rjno1Y +5, an = 1, w = 500 - 30 * 3 * 7, h = 20}
    lo.style = osc_styles.rjno140Buttons	
...
--an = 1 这样左侧的时间变长才会往右，但y轴要调整
    ...
    lo.geometry = {x = rjno1X + 40, y = rjno1Y + 10 * 1.6 + 120 / 2, an = 1, w = 110, h = 20}
    lo.style = osc_styles.rjno135Buttons	
...
--an = 9 这样右侧的时间变长才会往左，但y轴要调整
    ...
    lo.geometry = {x = posX + pos_offsetX -200, y = rjno1Y + 10 * 1.6 + 80 / 2, an = 6, w = 110, h = 20}
    lo.style = osc_styles.rjno135Buttons	
...
--an = 1 图标最左侧才能点击
    ...
    lo.geometry = {x = posX+pos_offsetX - 120, y = rjno1Y + 10 * 1.6 + 80 / 2, an = 5, w = 30, h = 50}
    lo.style = osc_styles.rjno150Buttons	
--an = 1 图标最左侧才能点击
    lo = add_layout("tog_fs")
    lo.geometry = {x = posX+pos_offsetX - 70, y = rjno1Y + 10 * 1.6 + 80 / 2, an = 5, w = 30, h =50 }
    lo.style = osc_styles.rjno150Buttons
